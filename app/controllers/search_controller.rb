class SearchController < ApplicationController
  ...
  def search
    ...
    @datas = search_for(@model, @value, @how) #search_forの引数にインスタンス変数を定義 #@datesに最終的な検索結果が入る
  end
  ...
  def match(model, value) #def search_forでhowがmatchだった場合の処理
    if model == 'user'    #modelがuserの場合の処理
      User.where(name:value) #whereでvalueと完全一致するnameを探している
    elsif model == 'book'
  ...
  def search_for(model, value, how) #searchアクションで定義した情報が引数に入っている
    ...
    when 'forward'                  #例えばhowがmatchの場合はdef matchの処理に進む
      forward(model, value)
    when 'backward'
